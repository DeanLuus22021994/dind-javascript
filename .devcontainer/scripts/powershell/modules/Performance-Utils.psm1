...
$script:PerformanceConfig = @{
    ...
    OptimizationSettings = @{
        MaxConcurrentBuilds = [Math]::Min(8, $script:ExportedConstants.MaxThreads)
        MemoryLimitMB = 4096
        IOPriority = 'Normal'
    ...
    [void]CollectMetrics() {
        ...
        try {
            ...
            $cpuMetric = @{
                Timestamp = $timestamp
                Usage = $cpuUsage
                Cores = $script:ExportedConstants.MaxThreads
            }
...
function Get-OptimalConcurrencySettings {
    ...
    # Base settings on current system resources
    $cpuCores = $systemInfo.Performance.MaxThreads
...
function Test-SystemPerformanceCapabilities {
    ...
    try {
        ...
        $cpuStart = Get-Date
        $cpuTest = 1..1000 | ForEach-Object -Parallel {
            [Math]::Sqrt($_) * [Math]::PI
        } -ThrottleLimit $script:ExportedConstants.MaxThreads
